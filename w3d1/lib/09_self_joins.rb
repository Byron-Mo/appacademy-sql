...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    ...
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
    JOIN
      stops transfer ON b.stop_id = transfer.id
    JOIN
      routes c ON (b.company = c.company AND b.num = c.num)
    JOIN
      stops destination ON c.stop_id = destination.id
    WHERE
      a.stop_id = 53
      AND
      destination.name = 'Sighthill'
  SQL
end
